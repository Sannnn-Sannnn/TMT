...
describe("userService.login", () => {
  ...
  it("should throw if the email does not exist", async () => {
    ...
    await expect(
      userService.login({ email: "nope@a.com", password: "plain-pw" })
  ...
  it("should throw if the password does not match", async () => {
    ...
    await expect(
      userService.login({ email: "a@a.com", password: "wrong-pw" })
    ).rejects.toThrow("Invalid email or password");
  });
});
...
describe("userService.logout", () => {
  it("should blacklist the token so a later findByToken call rejects it", async () => {
    ...
    await expect(userService.findByToken("token-to-blacklist", 1)).rejects.toThrow(
      "Invalid user"
    );
  });
});
